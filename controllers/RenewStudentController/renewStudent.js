...
module.exports = {
 //load form
      ...
      const enhancedClasses = await Promise.all(classes.map(async (cls, index) => {
        ...
        studentsWithExams.forEach(student => {
          ...
          const passedAll = Object.values(subjectResults).every(result =>
            result.subjectCodeRef?.subject &&
            result.marks >= result.subjectCodeRef.subject.passmarks
          );
  
          if (passedAll) passedStudents++;
          else failedStudents++;
        });
  ...
  promoteStudents: async (req, res) => {
    ...
    try {
      const { classId, sectionId, newClassId, newSectionId } = req.body;
      ...
      if (!currentHistories.length) throw new Error('No students found to promote');
      ...
      req.flash('success', `Successfully promoted ${currentHistories.length} students`);
